...
const char str_prog_name[] PROGMEM   = "\r\n\r\nXMEGA32A4U ADC_PA1_11bit_2V_REF MAP 16/01/2015..\r\n"; // Program name
    ...
    for(;;)
    {
		...
		if (prev_time != uptime)
		{
			...
			*/
			...
			_micros = TCD0.CNT;
			//ADC_Reading = ReadADC_16AVG(2, 1); // PA2 & Ext Signal
			ADC_Reading = ReadADC_16AVG(1, 1); // PA1 & Ext Signal
			_micros = TCD0.CNT - _micros;
			//!! For profile only!!
			//printf_P(PSTR(">>>ADC ADC_Reading speed is: %u us\r\n"), _micros);
			
			//!! Результат для 11-bit signed mode
			printf_P(PSTR("ADC PA.1(int) is: %d mV\r\n"), ADC_Reading); // Для опоры 3.3/1.6 - 1 единица измерения = 1 mv (3.3/1.6/2048)
			//printf_P(PSTR("ADC PA.2(float) %.1f mV\r\n"), (float)ADC_Reading*1.007); //Float(Accurate but slowest) Result in mV float (опора Vcc/1.6)
			

			//!! Результат для 8-bit unsigned mode (-ADC_8bit_offset для компенсации сдвига нуля)
			//printf_P(PSTR("ADC PA.2(int) is: %d mV\r\n"), ((ADC_Reading-ADC_8bit_offset)*8)); // Для опоры 3.3/1.6 - 1 единица измерения = 8.05 mv (3.3/1.6/256)
			//printf_P(PSTR("ADC PA.2(float) %.1f mV\r\n"), (float)(ADC_Reading-ADC_8bit_offset)*8.05); //Float(Accurate but slowest) Result in mV float (опора Vcc/1.6)
			

			//!! Результат для 12-bit unsigned mode (-ADC_12bit_offset для компенсации сдвига нуля)
			/*
			printf_P(PSTR("ADC PA.2(int) is: %d mV\r\n"), ((ADC_Reading-ADC_12bit_offset)/2)); // Для опоры 3.3/1.6 - 1 единица измерения = .503 mv (3.3/1.6/2048)
			printf_P(PSTR("ADC PA.2(float) %.1f mV\r\n"), (float)(ADC_Reading-ADC_12bit_offset)*0.503); //Float(Accurate but slowest) Result in mV float (опора Vcc/1.6)
			*/
