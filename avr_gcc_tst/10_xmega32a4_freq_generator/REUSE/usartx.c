	...
	while (1)							// do forever...
	{
		...
		if (kbhit())					// if char available now...
		{
			...
			switch  (c)					// based on the char...
			{
				...
				if (c < ' ')  break;	// if not printable, ignore it
				putchar(c);				// show the char
 ...
 */
  ...
  */  
	USART_Init(eUSARTD0, BAUDA_38400, BAUDB_38400);	// init USARTD0 to 38400, 8N1

	USART_Connect(eUSARTD0, STDOUT_FILENO);			// use USARTD0 for stdout
	USART_Connect(eUSARTD0, STDIN_FILENO);			// use USARTD0 for stdin
	USART_Connect(eUSARTD0, STDERR_FILENO);			// use USARTD0 for stderr
