...
const char PROGMEM str_prog_name[]    = "\r\n\r\nXMEGA32A4U PWM SINGLE SLOPE 6 CH TCCD0/TCCD1 18/01/2015..\r\n"; // Program name
...
void TCD1_PWM_SINGLE_SLOPE_init(void){
	...
	//!!PWM frequency = system clock / (prescaler * (PER + 1) => 32Mhz/(4*(65535+1)) = 122Hz
}
...
void avr_init(void)
{
	...
	*/
	...
	TCD1_PWM_SINGLE_SLOPE_init(); // 2 outs - thus timer type 1 (see AVR XMEGA mans)
...
//***************************Profiling micros: END
    ...
    for(;;)
    {
		...
		//TCD0.CCA = (uint16_t)map(MAP_Reading, 10, 1400, 0, 255);
		TCD0.CCABUF = (uint16_t)map(MAP_Reading, 10, 1400, 0, 100); // Правильнее писать через промежуточный буфер - TCxx.CCxBUF, тем самым избегаем glitch-effect при добавления нового значения PWM
		_delay_ms(100);
