/*
 * TCD0 + TCD1 used for 32bit hardware _micros - overflow in ~4294.97sec (~ 71.58 min)
 * Профайлинг точности:
	>>>10us is: 12 us - 20% погрешности
	>>>100us is: 102 us - 2%погрешности
	>>>1ms is: 1004 us - 0.4% погрешности
	>>>65ms is: 65118 us - 0.2% погрешности
Вывод данной точности таймера для подсчета временных интервалов ~ от 20us (50kHz c погрешностью 5%) - 100us(10kHz - c погрешностью 2%)

PS. для отсчета малых временных интервалов <65ms 16bit (с точностью до 1us) применяем макрос
_micros_16 (им можем мерять от единиц микросекунд с достаточно высокой точностью)
*/
...
const char PROGMEM str_prog_name[]    = "\r\n>>>FROM BASE TEMPLATE\r\n\r\nXMEGA32A4U 32bit timer from TCD0+TCD1 for _micros() Timing 23/01/2015..\r\n"; // Program name
...
uint32_t  _millis(void); // Return number of milliseconds from TCE0 timer
uint32_t  _micros(void); // Return number of microseconds from TCD0 && TCD1 timer
void RTC_RC32K_1SEC_INIT(void);
void TCE0_init_millis(void);
void TCD0_TCD1_init_micros_32bit(void);
//****Function declaration: END


// _micros() defines
//!! For 16bit micros если достаточно 16-ти битного результата (переполнение каждые  ~65ms)
//!! - т.к. он считывается практически мнгновенно, и дает небольшую погрешность в пределах 1us
#define _micros_16() TCD0.CNT 
...
void TCD0_TCD1_init_micros_32bit(void)
{
 //!!TCD0 + TCD1 used for 32bit hardware _micros - overflow in ~4294.97sec (~ 71.58 min)
  ...
  // Event channel 1 MUX from TCD0 Overflow
  EVSYS.CH1MUX = EVSYS_CHMUX_TCD0_OVF_gc;

  TCD1.CNT = 0; // Считаем максимум до 65535us = ~ 65ms (Счетчики XMEGA 16-ти битные)
  TCD1.CTRLA = TC_CLKSEL_EVCH1_gc;  // clk from Event channel 1
  ...
  // configure timers/counters D0 && D1 to counting micros() (like Arduino)
...
// Return number of microseconds from timers TCD0 && TCD1
uint32_t  _micros(void)
{
	static uint16_t micros_low;
	static uint16_t micros_hi;
	static uint32_t micros_32bit;
	micros_hi = TCD1.CNT;
	micros_low = TCD0.CNT;
	// Это для обеспечения атомарного чтения 32-х битного числа без запрета прерываний (cli() ... sei())
	while(micros_hi != TCD1.CNT)
	{
		micros_hi = TCD1.CNT;
		micros_low = TCD0.CNT;
	}
	micros_32bit = (uint32_t)micros_hi*0x10000 + (uint32_t)micros_low;
	return micros_32bit;
}
...
void avr_init(void)
{
	...
	*/
	...
	//!! TCD0 && TCD1 setup for _micros() purposes
	//!! Precise err 0% overflow every ~2.2min (32bit hardware TCD0.CNT && TCD1.CNT)
	TCD0_TCD1_init_micros_32bit();	
...
//***************************Profiling micros: END
	...
	while(1)
	{
		...
		if(prev_time != _seconds())
		{
			...
			printf_P(PSTR("Micros is: %lu usec\r\n"), _micros());
		}
