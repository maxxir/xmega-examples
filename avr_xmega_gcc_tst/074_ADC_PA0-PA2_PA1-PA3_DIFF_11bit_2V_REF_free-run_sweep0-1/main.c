/*
 * 074_ADC_PA0-PA2_PA1-PADGND_DIFF_11bit_2V_REF_free-run_sweep0-1
 * Дифференциальный (2-х входовой режим) CH0(PA0+ & PA2-) && CH1(PA1+ & PA3-) 12 бит относительно <0> +-11bit(+-2047)
 * PA2- && PADGND(PA3)- заземлены на <GND> схемы
 * PA0 && PA1 - входа замера
*/
/*
+ ADC Channel CH0/CH1 (можно до 4-х каналов одновременно) PA1/PA2 autorun & sweep mode
* PS. за счет конвейерной архитектуры оба(до 4-х) канала обрабатываются практически одновременно со сдвигом на один такт ADC цикла
* (не видел такого не у МICROCHIP не у STM32)
* а можно включить 4 канала на такой метод семплирования!!
* Оч. удобно можно не заморачиваться с ожиданием флагов и сразу считывать результат преобразования (при частотах семплирования < ~20Khz)
* ADCA.CH0RES - читаем из канала 0 / ADCA.CH1RES - читаем из канала 1 /
* ADCA.CH2RES - читаем из канала 2 / ADCA.CH3RES - читаем из канала 3
* доп. инфо читать <ATMEL AVR1300: Using ATMEL AVR XMEGA ADC>

+ ADC_PA1_11bit_REF_2V
...
const char str_prog_name[] PROGMEM   = "\r\n\r\n074_ADC_PA0-PA2_PA1-PADGND_DIFF_11bit_2V_REF_free-run_sweep0-1 26/01/2015..\r\n"; // Program name
...
void ADC_init_11bit_signed_CH0_CH1_sweep(void);
void ADC_init_DIFF_11bit_signed_CH0_CH1_sweep(void);
...
void ADC_init_8bit(void)
{
        ...
        //!! Но отриц. напряжение не меряет - до -500mV (подавал от AAA батарейки +-1.5V , В отриц. полярность показывает ~-500mV)
...
// Вкл. FREE-RUN mode and CHANNEL SWEEP 0-1, т.е. при достаточно низкой частоте семплирования
// ~ >20Khz можно вообще не смотреть флаг прерывания окончания семплирования - все считывается автоматом
// ADCA.PRESCALER = ADC_PRESCALER_DIV128_gc ; // ~ 30us for 1 samling - OK ~ 33kHz
// PS. можем включить SWEEP 0-1-2-3 канала и  считывать значения одновременно (у STM32 и MICROCHIP ТАКОГО НЕ ВИДЕЛ)
...
//!! Внимание отриц. напряжение - относительно <GND> ~0.4V все что ниже обрезается!!!
void ADC_init_11bit_signed_CH0_CH1_sweep(void)
{
	    // Переназначаем ADC входа в режим входа && Отключаем цифровой режим
	    PORTA.DIRCLR = 1<<2; //(PORTA2 as ADC iniput)
	    ...
	    PORTA.DIRCLR = 1<<1; //(PORTA1 as ADC iniput)
	    PORTA.PIN1CTRL = PORT_ISC_INPUT_DISABLE_gc; // Disable Digital Input Buffer for PA2
        ...
        //ADCA.CTRLB = (1<<4)|ADC_RESOLUTION_12BIT_gc; // Signed Mode 11 bit 
        ADCA.CTRLB = ADC_CONMODE_bm|ADC_RESOLUTION_12BIT_gc|ADC_FREERUN_bm; // Signed Mode 11 bit  && Free RUN
        //!! GOOD MODE
        //!! Меряем 11bit signed right adjustened - при этом меряем +- 2^^11 бит от <0> -("Мертвая" зона см. <Unsigned Mode 12 bit> отсутствует!!)
       //!! Но отриц. напряжение не меряет - до -500mV (подавал от AAA батарейки +-1.5V , В отриц. полярность показывает ~-500mV)
        ...
        //ADCA.EVCTRL = ADC_EVACT_NONE_gc ; // no events
        ADCA.EVCTRL = ADC_SWEEP_01_gc; // Channel sweep 0-1
        ADCA.PRESCALER = ADC_PRESCALER_DIV128_gc ; // ~ 30us for 1 samling - OK
		//ADCA.PRESCALER = ADC_PRESCALER_DIV64_gc; // ~15us for 1 sampling - OK
		//ADCA.PRESCALER = ADC_PRESCALER_DIV32_gc; // ~9us for 1 sampling - OK
		//ADCA.PRESCALER = ADC_PRESCALER_DIV16_gc; // ~5us for 1 sampling (Здесь и далее уже играет роль импенданс входной цепи начали "заваливать результат на ~ 2%" на сопротивлении 10К)
		...
		// For READ ADC.CH0 - PA.1 - as default in CH0
		ADCA.CH0.CTRL = ADC_CH_GAIN_1X_gc | ADC_CH_INPUTMODE_SINGLEENDED_gc ; // Gain = 1, Single Ended
		ADCA.CH0.MUXCTRL = ADC_CH_MUXPOS_PIN1_gc; // (1<<3) Input PIN- PA1 for ADCA.CH0
		ADCA.CH0.INTCTRL = ADC_CH_INTLVL_OFF_gc ; // No interrupt

		// For READ ADC.CH1 - PA.2 - as default in CH1
		ADCA.CH1.CTRL = ADC_CH_GAIN_1X_gc | ADC_CH_INPUTMODE_SINGLEENDED_gc ; // Gain = 1, Single Ended
		ADCA.CH1.MUXCTRL = ADC_CH_MUXPOS_PIN2_gc; // (2<<3) Input PIN- PA2 for ADCA.CH1
		ADCA.CH1.INTCTRL = ADC_CH_INTLVL_OFF_gc ; // No interrupt

}


#define ADC_CH_MUXNEGL_GND_gc			0x05	// DIFF NO GAIN - PAD GND
#define ADC_CH_MUXNEGL_INTGND_gc		0x07	// DIFF NO GAIN - INTERNAL GND
...
// GOOD считаем +-2**11 = +-2048 MAX diff-ended c опорой 1/1.6/2V
// погрешность - ОК ~ 1% и 0 ~ соответствует.
// !! Шаг замера 1mv для диапазона [0-2V] c погрешностью ~1%
//!! Предварительная калибровка <0> НЕ НУЖНА (в отличии от ADC_init_8bit && ADC_init_12bit)
//!! Внимание отриц. напряжение - относительно <GND> ~0.4V все что ниже обрезается!!!
...
{
	    // Переназначаем ADC входа в режим входа && Отключаем цифровой режим
	    PORTA.DIRCLR = 1<<0; //(PORTA0 as ADC iniput)
	    PORTA.PIN0CTRL = PORT_ISC_INPUT_DISABLE_gc; // Disable Digital Input Buffer for PA0

	    PORTA.DIRCLR = 1<<1; //(PORTA1 as ADC iniput)
	    PORTA.PIN1CTRL = PORT_ISC_INPUT_DISABLE_gc; // Disable Digital Input Buffer for PA1

	    PORTA.DIRCLR = 1<<2; //(PORTA2 as ADC iniput)
	    PORTA.PIN2CTRL = PORT_ISC_INPUT_DISABLE_gc; // Disable Digital Input Buffer for PA2

	    PORTA.DIRCLR = 1<<3; //(PORTA3 as ADC iniput)
	    PORTA.PIN3CTRL = PORT_ISC_INPUT_DISABLE_gc; // Disable Digital Input Buffer for PA3
       ...
       //!! Но отриц. напряжение не меряет - до -500mV (подавал от AAA батарейки +-1.5V , В отриц. полярность показывает ~-500mV)
		...
		//!! Внимание для DIFF mode no GAIN в кач-ве NEG INPUTS PA0-PA3 
		//!! (или ADC_CH_MUXNEGL_GND_gc - PAD GROUND (0x05) 
		//!! или ADC_CH_MUXNEGL_INTGND_gc - INTERNAL GROUND (0x07))
		
		// For READ ADC.CH0 - [PA.0+ PA.2-] - as default in CH0 in DIFF mode
		//ADCA.CH0.CTRL = ADC_CH_GAIN_1X_gc | ADC_CH_INPUTMODE_SINGLEENDED_gc ; // Gain = 1, Single Ended
		ADCA.CH0.CTRL = ADC_CH_GAIN_1X_gc | ADC_CH_INPUTMODE_DIFF_gc ; // Gain = 1, DIFF Ended no GAIN
		ADCA.CH0.MUXCTRL = ADC_CH_MUXPOS_PIN0_gc | ADC_CH_MUXNEG_PIN2_gc; // Input PIN- PA0+ && PA2- for ADCA.CH0
		ADCA.CH0.INTCTRL = ADC_CH_INTLVL_OFF_gc ; // No interrupt

		// For READ ADC.CH1 - [PA.1+ PA.3-] - as default in CH1 in DIFF mode
		//ADCA.CH1.CTRL = ADC_CH_GAIN_1X_gc | ADC_CH_INPUTMODE_SINGLEENDED_gc ; // Gain = 1, Single Ended
		ADCA.CH1.CTRL = ADC_CH_GAIN_1X_gc | ADC_CH_INPUTMODE_DIFF_gc ; // Gain = 1, DIFF Ended no GAIN
		ADCA.CH1.MUXCTRL = ADC_CH_MUXPOS_PIN1_gc | ADC_CH_MUXNEGL_GND_gc; // Input PIN- PA1+ && PAD GND (0x05) for ADCA.CH1
		//ADCA.CH1.MUXCTRL = ADC_CH_MUXPOS_PIN1_gc | ADC_CH_MUXNEG_PIN3_gc; // Input PIN- PA1+ && PA3- for ADCA.CH1
		ADCA.CH1.INTCTRL = ADC_CH_INTLVL_OFF_gc ; // No interrupt
...
void avr_init(void)
{
	...
	*/
	...
	//ADC_init_11bit_signed_CH0_CH1_sweep(); // BEST && Accurate ResulT (1step = 1mv)  - NOT NEED <0> calibration - ~1%err - [0-2V] measure
	ADC_init_DIFF_11bit_signed_CH0_CH1_sweep(); // The same as above - only differential input. BEST && Accurate ResulT (1step = 1mv)  - NOT NEED <0> calibration - ~1%err - [0-2V] measure
    ...
    for(;;)
    {
		...
		if (prev_time != uptime)
		{
			...
			*/
			...
			// Use profile for speed ADC_Reading measure
			//_micros = TCD0.CNT;
			//_micros = TCD0.CNT - _micros;
			//!! For profile only!!
			//printf_P(PSTR(">>>ADC ADC_Reading speed is: %u us\r\n"), _micros);
			
			//!! Результат для 11-bit signed mode
			// Т.к. считываем раз в 1 сек и включен FREE-RUN && SWEEP0-1 сразу считываем результат преобразования (скорость семпрлирования ~30kHz)
			// ТАКИМ ОБРАЗОМ МОЖЕМ СЧИТЫВАТЬ ПОКАЗАНИЯ 4-Х КАНАЛОВ ПРЕОБРАЗОВАНИЯ (оч. удобная фишка XMEGA-A)
			printf_P(PSTR("\r\nADCA.CH0 PA.0(int) is: %d mV\r\n"), ADCA.CH0RES); // Для опоры 3.3/1.6 - 1 единица измерения = 1 mv (3.3/1.6/2048)
			printf_P(PSTR("ADCA.CH1 PA.1(int) is: %d mV\r\n"), ADCA.CH1RES); // Для опоры 3.3/1.6 - 1 единица измерения = 1 mv (3.3/1.6/2048)
