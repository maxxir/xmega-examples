...
const char str_prog_name[] PROGMEM   = "\r\n\r\nXMEGA32A4U ADC_PA2_8bit-11bit_1V_REF 9/01/2015..\r\n"; // Program name
...
// VAR2 for READ ADC with auto setup
    ...
    for(;;)
    {
		...
		if (prev_time != uptime)
		{
			...
			*/
			...
			// Use profile for speed reading measure
			_micros = TCD0.CNT;
			Reading = ReadADC(2, 1); // CH2 & Ext Signal
			_micros = TCD0.CNT - _micros;
			//!! For profile only!!
			printf_P(PSTR(">>>ADC reading speed is: %u us\r\n"), _micros);
			
			//!! Результат для 11-bit signed mode
			printf_P(PSTR("ADC PA.2(int) is: %d mV\r\n"), Reading); // Для опоры 3.3/1.6 - 1 единица измерения = 1 mv (3.3/1.6/2048)
			printf_P(PSTR("ADC PA.2(float) %.1f mV\r\n"), (float)Reading*1.007); //Float(Accurate but slowest) Result in mV float (опора Vcc/1.6)
			

			//!! Результат для 8-bit unsigned mode (-ADC_8bit_offset для компенсации сдвига нуля)
			//printf_P(PSTR("ADC PA.2(int) is: %d mV\r\n"), ((Reading-ADC_8bit_offset)*8)); // Для опоры 3.3/1.6 - 1 единица измерения = 8.05 mv (3.3/1.6/256)
			//printf_P(PSTR("ADC PA.2(float) %.1f mV\r\n"), (float)(Reading-ADC_8bit_offset)*8.05); //Float(Accurate but slowest) Result in mV float (опора Vcc/1.6)
			

			//!! Результат для 12-bit unsigned mode (-ADC_12bit_offset для компенсации сдвига нуля)
			/*
			printf_P(PSTR("ADC PA.2(int) is: %d mV\r\n"), ((Reading-ADC_12bit_offset)/2)); // Для опоры 3.3/1.6 - 1 единица измерения = .503 mv (3.3/1.6/2048)
			printf_P(PSTR("ADC PA.2(float) %.1f mV\r\n"), (float)(Reading-ADC_12bit_offset)*0.503); //Float(Accurate but slowest) Result in mV float (опора Vcc/1.6)
			*/
			
			/*
			//!!ADC Reading VAR3 (as previous but inline)
			// Use profile for speed reading measure
			_micros = TCD0.CNT;
