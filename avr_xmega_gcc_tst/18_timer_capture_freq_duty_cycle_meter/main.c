/*
+ TCC0 CCA use for input capture via OC0A/PC0 (captute channel)
+ TCD0 CCA/CCB/CCC/CCD used for generate PWM pattern signals 122Hz with duty cycle: 3%/12%/49%/94%
+ TCD1 CCA/CCB used for generate PWM pattern signals 488Hz with duty cycle: 24%/62%
*/
...
const char PROGMEM str_prog_name[]    = "\r\n>>>BASE TEMPLATE\r\n\r\nXMEGA32A4U TCC0 CCA PC0 capture freq && duty cycle measure 23/01/2015..\r\n"; // Program name
...
void TCD0_PWM_SINGLE_SLOPE_init(void);
void TCD1_PWM_SINGLE_SLOPE_init(void);
void PWM_PINs_init(void);
void TCC0_CCA_PC0_init_capture(void);
...
// VARs for input capture
volatile uint32_t frequency;
volatile uint32_t dutyCycle;
volatile uint16_t totalPeriod;
volatile uint16_t highPeriod;
volatile uint8_t sem_capture_ok; // Semaphore capture is OK
//Init Input capture via TCC0 channel CCA pin PC0
void TCC0_CCA_PC0_init_capture(void)
{
	/* Configure PC0 for input, triggered on both edges. */
	PORTC.PIN0CTRL = PORT_ISC_BOTHEDGES_gc;
	PORTC.DIRCLR = 0x01;
	PORTC.PIN0CTRL |= PORT_OPC_PULLUP_gc; // Totempole w/ Pull-up on Input  PULL-UP

	/* Select PC0 as input to event channel 0. */
	EVSYS.CH0MUX = EVSYS_CHMUX_PORTC_PIN0_gc;

	/* Configure TCC0 for Input Capture using event channel 0. */
	...
	TCC0.PER = 0x7FFF; // 15 bit == 0 on PER => 15bit CCx из edge polarity

	// 1us timer TCC0 tick
	EVSYS.CH1MUX = EVSYS_CHMUX_PRESCALER_32_gc; // Prescaler (from SYSCLK) divide by 32 so - 1us(for 32Mhz SYSCLK)!!
	TCC0.CTRLA = TC_CLKSEL_EVCH1_gc;  // clk from Event channel 1
	...
	//PMIC.CTRL |= PMIC_LOLVLEN_bm; //Enabled in avr_init()
}

// ISR for Input capture TCC0 CCA
ISR(TCC0_CCA_vect)
{

	// Get Captured value
	uint16_t thisCapture = TCC0.CCA;

	/*  Save total period based on rising edge and reset counter. */
	if ( thisCapture & 0x8000 ) {
		totalPeriod = thisCapture & 0x7FFF;
		//TC_Restart( &TCC0 );
		TCC0.CTRLFSET = TC_CMD_RESTART_gc; // Restart the timer
	}
	...
	dutyCycle = ( ( ( ((uint32_t)highPeriod) * 100 ) / ((uint32_t)totalPeriod) ) + dutyCycle ) / 2;
	frequency = ( ( ( F_CPU / 32 ) / totalPeriod ) + frequency ) / 2;
	sem_capture_ok = 1;
}
...
void TCD1_PWM_SINGLE_SLOPE_init(void){
	...
	//!!PWM frequency = system clock / (prescaler * (PER + 1) => 32Mhz/(1*(65535+1)) = 488Hz
}
...
void TCD0_PWM_SINGLE_SLOPE_init(void){
	...
	//!!PWM frequency = system clock / (prescaler * (PER + 1) => 32Mhz/(1*(255+1)) = 125kHz
	
	TCD0.CTRLA = TC_CLKSEL_DIV1024_gc;
	//!!PWM frequency = system clock / (prescaler * (PER + 1) => 32Mhz/(1024*(255+1)) = 122Hz
...
void avr_init(void)
{
	...
	*/
	...
	//!! TCD0 setup for _micros() purposes
	//!! Precise err 0% overflow every ~65msec (16bit hardware TCD0.CNT)
	//TCD0_init_micros();	//Not used here
	...
	PWM_PINs_init();
	//!! SETUP PWM outs
	TCD0_PWM_SINGLE_SLOPE_init(); // 4 outs - thus timer type 0 (see AVR XMEGA mans)
	TCD1_PWM_SINGLE_SLOPE_init(); // 2 outs - thus timer type 0 (see AVR XMEGA mans)
	// Set all PWM outputs to some value (8bits resolution pwm)
	TCD0.CCA = 0x09; //3.5% Duty Cycle
	TCD0.CCB = 0x1f; //12%  Duty Cycle
	TCD0.CCC = 0x7f; //49.8% Duty Cycle
	TCD0.CCD = 0xf0; //94.1% Duty Cycle
	TCD1.CCA = 0x3FFF; //24.9% Duty Cycle
	TCD1.CCB = 0x9FFF; //62.5%  Duty Cycle
	
	//!!SET-UP PC0 CCA PC0 input capture
	TCC0_CCA_PC0_init_capture();
...
int main(void)
{
	...
	while(1)
	{
		...
		if(prev_time != _seconds())
		{
			...
			// Check if capture result is valid
			cli(); // For atomic reading
			if (sem_capture_ok)
			{
				sem_capture_ok = 0; // Clear semaphore
				if(!(TCC0.INTFLAGS & TC0_ERRIF_bm))
				{
					//OK
					//PWM signals measure result
					printf_P(PSTR("FREQUENCY on PC0 is: %lu Hz\r\n"), frequency);
					printf_P(PSTR("DUTY cycle on PC0 is: %lu %%\r\n"), dutyCycle);
				}
				else
				{
					// Error, clear the error flag
					TCC0.INTFLAGS = TCC0.INTFLAGS | TC0_ERRIF_bm;
					printf_P(PSTR("FREQUENCY on PC0 is: ERROR Hz\r\n"));
					printf_P(PSTR("DUTY cycle on PC0 is: ERROR %%\r\n"));
				}
			}//if (sem_capture_ok)
			else
			{
				printf_P(PSTR("FREQUENCY on PC0 is: NONE Hz\r\n"));
				printf_P(PSTR("DUTY cycle on PC0 is: NONE %%\r\n"));
			}//if (sem_capture_ok) else..
			sei();
